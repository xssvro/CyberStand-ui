...
type DatePickerPlacement = 'bottom-start' | 'top-start' | 'auto';
...
function applyPanelPosition(
  ...
  let openUp =
    placement === 'top-start' ||
    (placement === 'auto' && below < panelH && above > below);
...
export const DatePicker = forwardRef<HTMLButtonElement, DatePickerProps>(function DatePicker(
  ...
  useLayoutEffect(() => {
    ...
    const run = () => {
      ...
      if (p && t) applyPanelPosition(p, t, placement);
    };
    run();
  ...
  const panel = open ? (
    ...
    >
      ...
      <div id={listboxId} className={styles.grid}>
        {cells.map((cell) => {
          const selectable = isDateInRange(cell.iso, min, max);
